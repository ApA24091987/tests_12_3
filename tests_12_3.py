...
# Настройка логирования для тестов Runner
logging.basicConfig(
    level=logging.INFO,
...
# Декоратор для пропуска тестов при is_frozen=True
...
class RunnerTest(unittest.TestCase):
    is_frozen = False  # Атрибут для управления выполнением тестов

    @check_frozen
    def test_walk(self):
        try:
            runner = Runner("Alice", -5)  # Отрицательная скорость
            runner.walk()
...
class TournamentTest(unittest.TestCase):
    is_frozen = True  # Атрибут для управления выполнением тестов
